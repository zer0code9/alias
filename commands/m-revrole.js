...
function revRole(msg, args) {
    ...
    if (user.roles.cache.has(`${role.id}`)) return Invalid(msg, `Doesn't have Role`, `The member ${user} doesn't have the role ${role}`, `addrole [member] [role]`);
    
    user.remove(`${role}`);
    const revRole = new Discord.MessageEmbed()
    .setColor('#00ff00')
...
module.exports = {
    ...
    type: "moderation",
    execute(msg, args) {
        if (args[0]) {return revRole(msg, args)}
        if (!msg.member.hasPermission("MANAGE_ROLES")) return Perm(msg, `No Permission`, `You don't have the permission to manage roles`);
        if (!msg.guild.me.hasPermission("MANAGE_ROLES")) return Perm(msg, `No Permission`, `I don't have the permission to manage roles`);
