...
const { ApplicationCommandOptionType, Message, ChatInputCommandInteraction, GuildMember, User, Guild } = require('discord.js');
...
module.exports = {
    ...
    async msgRun(msg, args) {
        ...
        if (action == "create") {
            ...
            const member = msg.guild.members.cache.get(args[2]) ?? msg.guild.members.cache.get(msg.mentions.users.first()?.id);
            const reason = args.slice(3).join(" ");

            try {
                const Create = await this.Create(option, issuer, member, reason);
                AliasSends.sendEmbedAlias(msg, Create);
                AliasSends.sendEmbedUser(member, AliasUtils.getMesage(member, `banned`, reason));
            } catch {
        ...
        if (action == "delete") {
            const issuer = msg.member;
            const user = msg.guild.members.cache.get(args[1])?.user ?? msg.guild.members.cache.get(msg.mentions.users.first()?.id)?.user;
    ...
    async intRun(int) {
        ...
        if (action == "create") {
            const option = int.options.getSubcommand();
     ...
     */
    async Create(option, issuer, member, reason) {
        ...
        if (!member) return AliasEmbeds.invalid(`No User`, `I need a member in order to ban someone \n(${options.options[0].options[0].specific})`, AliasUtils.getUsage(this, "create", option));
    ...
    async Delete(issuer, user, reason) {
        ...
        if (!user) return AliasEmbeds.invalid(`No Id`, `I need a valid id in order to unban someone \n(${options.options[0].specific})`, AliasUtils.getUsage(this, "delete"));
